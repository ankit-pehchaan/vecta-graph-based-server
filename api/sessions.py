...
Now integrates with PostgreSQL for user-owned data persistence.
"""
...
class SessionManager:
    ...
    Sessions are now linked to users, with data persisted to PostgreSQL.
    """
    ...
    def create_session(
        ...
        If user_id is provided, loads user's existing profile data.
        """
        # Generate session ID
        session_id = os.urandom(16).hex()
        
        # Create DB session record if user is authenticated
        if user_id:
            try:
                db_session_id = self.session_repo.create_session(user_id)
                session_id = db_session_id  # Use DB-generated UUID
            except Exception:
                # Fallback to random ID if DB fails
                pass
        ...
        # Load user's existing data if authenticated
        if user_id:
    ...
    def _load_user_data(self, orchestrator: Orchestrator, user_id: int) -> None:
        ...
        try:
            ...
            for node_name, data in node_data.items():
                ...
                # Mark nodes with data as visited
                if data:
    ...
    def persist_session(self, session_id: str) -> None:
        ...
        Called after each user turn to save changes.
        """
