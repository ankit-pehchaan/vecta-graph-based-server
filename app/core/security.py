...
def get_token_from_websocket(query_params: dict, headers: dict) -> str | None:
    """
    Extract JWT token from WebSocket connection.
    
    Checks query parameters first (token=...), then Authorization header.
    
    Args:
        query_params: WebSocket query parameters dict
        headers: WebSocket headers dict
...
def decode_token_from_websocket(query_params: dict, headers: dict, token_type: str = "access") -> dict[str, str | int] | None:
    """
    Extract and decode JWT token from WebSocket connection.
    ...
    """
    token = get_token_from_websocket(query_params, headers)
    if not token:
        return None
